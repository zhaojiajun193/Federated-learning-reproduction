...
def start_train_end_node_process_without_print(x_train_dict, y_train_dict, beta_1, tao, learning_rate, learning_rate_of_clients, momentum):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_size = 100
    ...
    for fd_epoch in range(10000):
        ...
        main_model, v_t, real_delta_t_dict = get_new_main_model(main_model, real_delta_t_dict, selected_client, model_dict , beta_1,  v_t, tao, learning_rate)
...
trainset = torchvision.datasets.CIFAR100(root='./cifar100', train=True, download=True, transform=torchvision.transforms.ToTensor())
beta_1 = 0
tao = 0.01
momentum = 0.9
learning_rate = 0.1
...
'''
start_train_end_node_process_without_print(image_data_dict, label_dict, beta_1, tao, learning_rate_fd, learning_rate, momentum)