...
def start_train_end_node_process_without_print(x_train_dict, y_train_dict, beta_1, tao, learning_rate, learning_rate_of_clients, momentum, beta_2):
    ...
    testset = torchvision.datasets.EMNIST(root='./emnist', train=False,
                                           download=True, transform=transform, split='mnist')
    ...
    for fd_epoch in range(10000):
        ...
        for member in selected_client:
            ...
            for epoch in range(10):
                ...
                out = toPIL(out)
                out.save('member' + str(member) + 'epoch' + str(epoch) + '.jpg')
        ...
        with torch.no_grad():
            for data in testloader:
                ...
                each_testloss = main_model_loss(outputs, images)
                testloss += each_testloss
                # the class with the highest energy is what we choose as prediction
                #_, predicted = torch.max(outputs.data, 1)
                #total += labels.size(0)
                #correct += (predicted == labels).sum().item()
        print('轮次',fd_epoch, '损失', testloss, '单一损失', testloss/len(testset))
        if fd_epoch%100==0:
            toPIL = transforms.ToPILImage()
