...
def send_main_model_to_nodes_and_update_model_dict(main_model, model_dict, selected_client):
    with torch.no_grad():
        for i in selected_client:
            model_dict['model' + str(i)].encoder[0].weight.data = main_model.encoder[0].weight.data.clone()
            model_dict['model' + str(i)].encoder[0].bias.data = main_model.encoder[0].bias.data.clone()

            model_dict['model' + str(i)].encoder[2].weight.data = main_model.encoder[2].weight.data.clone()
            model_dict['model' + str(i)].encoder[2].bias.data = main_model.encoder[2].bias.data.clone()

            model_dict['model' + str(i)].encoder[4].weight.data = main_model.encoder[4].weight.data.clone()
            model_dict['model' + str(i)].encoder[4].bias.data = main_model.encoder[4].bias.data.clone()

            model_dict['model' + str(i)].encoder[6].weight.data = main_model.encoder[6].weight.data.clone()
            model_dict['model' + str(i)].encoder[6].bias.data = main_model.encoder[6].bias.data.clone()

            model_dict['model' + str(i)].decoder[0].weight.data = main_model.decoder[0].weight.data.clone()
            model_dict['model' + str(i)].decoder[0].bias.data = main_model.decoder[0].bias.data.clone()

            model_dict['model' + str(i)].decoder[2].weight.data = main_model.decoder[2].weight.data.clone()
            model_dict['model' + str(i)].decoder[2].bias.data = main_model.decoder[2].bias.data.clone()

            model_dict['model' + str(i)].decoder[4].weight.data = main_model.decoder[4].weight.data.clone()
            model_dict['model' + str(i)].decoder[4].bias.data = main_model.decoder[4].bias.data.clone()

            model_dict['model' + str(i)].decoder[6].weight.data = main_model.decoder[6].weight.data.clone()
            model_dict['model' + str(i)].decoder[6].bias.data = main_model.decoder[6].bias.data.clone()
...
def start_train_end_node_process_without_print(x_train_dict, y_train_dict, beta_1, beta_2, tao, learning_rate, learning_rate_for_client, momentum):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_size = 100
    ...
    for fd_epoch in range(10000):
        ...
        model_dict, optimizer_dict, criterion_dict = create_model_optimizer_criterion_dict(selected_client, learning_rate_for_client, momentum)
        model_dict = send_main_model_to_nodes_and_update_model_dict(main_model, model_dict, selected_client)
        #print('main_model', main_model.encoder[0].weight.data)
        running_loss = 0.0
        for member in selected_client:
            ...
            #print('client model first', member, model.encoder[0].weight.data)
            ...
            for epoch in range(1):
                ...
                print('模型损失', running_loss)
            model_dict.update({'model' + str(member):model})
            #print('client model after update', model.encoder[0].weight.data)
                    #print(fd_epoch, running_loss, member)
        ...
        #print('更新',main_model.encoder[0].weight.data)
        #print('vt',v_t['encoder.0.weight'])
        #print('deltat', real_delta_t_dict['encoder.0.weight'])
        ...
        with torch.no_grad():
            for data in testloader:
                ...
                running_loss += loss.item()
        print(fd_epoch, running_loss, running_loss/len(testset))
        toPIL = transforms.ToPILImage()
        img, label = testset[1502]
        img = toPIL(img)
        img.save('r123.jpg')
        img, label = testset[1502]
        img = img.to(device)
        out = main_model(img.reshape(28 * 28)).reshape(1, 28, 28)
        out = toPIL(out)
        out.save(str(fd_epoch) + '.jpg')
...
train_dict = select_client(10, 500)
...
'''
start_train_end_node_process_without_print(image_data_dict, label_dict,  beta_1, beta_2, tao, learning_rate_fd, learning_rate, momentum)